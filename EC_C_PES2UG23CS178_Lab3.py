...
def get_avg_info_of_attribute(data: np.ndarray, attribute: int) -> float:
    ...
    if attribute < 0 or rows == 0 or attribute >= cols - 1:
        return 0.0
    ...
    for val in unique_values:
        mask = attr_col == val
        subset = data[mask]
        sub_rows = subset.shape[0]
        if sub_rows == 0: continue
        weight = sub_rows / rows
...
def get_information_gain(data: np.ndarray, attribute: int) -> float:
    ...
    return round(get_entropy_of_dataset(data) - get_avg_info_of_attribute(data, attribute), 4)
...
def get_selected_attribute(data: np.ndarray) -> tuple:
    ...
    max_info_gain = -1
    sel_attr = 0
    attr_cols = data.shape[1] - 1

    for attr in range(attr_cols):
        info_gain = get_information_gain(data, attr)
        gain_dict[attr] = info_gain
        if info_gain > max_info_gain:
            max_info_gain = info_gain
            sel_attr = attr

    return (gain_dict, sel_attr) if gain_dict else ({}, -1)